import { IMapsSearchProvider, RawMapsBusiness, MapsSearchOptions } from './MapsSearchProvider.interface';

interface MockBusinessTemplate {
  namePrefixes: string[];
  nameSuffixes: string[];
  categories: string[];
  streets: string[];
  hasWebsiteProb: number;
  hasPhoneProb: number;
}

const MOCK_MAPS_TEMPLATES: Record<string, MockBusinessTemplate> = {
  cafe: {
...
export class MockMapsProvider implements IMapsSearchProvider {
  readonly providerName = 'MockMapsProvider';

  async search(query: string, location: string, options?: MapsSearchOptions): Promise<RawMapsBusiness[]> {
    ...
    const businesses: RawMapsBusiness[] = [];
    let count = 0;
