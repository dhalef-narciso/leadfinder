...
export interface GeneratedQuery {
  query: string;
  location: string;
  operatorType: 'instagram' | 'contact' | 'exclude_social' | 'exclude_website' | 'booking' | 'standard';
  description: string;
}

export class QueryGeneratorService {
  // Common synonym map for popular niches
  private static defaultSynonyms: Record<string, string[]> = {
  ...
  static getSynonymsForNiche(niche: string, customSynonyms?: string[]): string[] {
   ...
   */
  static generateMapsQueries(niche: string, customSynonyms?: string[], limit: number = 4): string[] {
    const synonyms = this.getSynonymsForNiche(niche, customSynonyms);
    return synonyms.slice(0, Math.min(synonyms.length, limit));
  }

  static generate(options: QueryGenOptions): GeneratedQuery[] {
