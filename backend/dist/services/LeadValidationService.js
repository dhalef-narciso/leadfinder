...
exports.LeadValidationService = void 0;
...
class LeadValidationService {
     ...
     */
    static async validateLead(lead, options) {
        const evidence = [];
        ...
        if (lead.websiteUrl) {
            websiteRecord = await UrlValidationService_1.UrlValidationService.validateUrl(lead.websiteUrl, 'website', {
                isDemo,
        ...
        if (lead.instagram) {
            igValidation = await SocialValidationService_1.SocialValidationService.validateInstagram(lead.instagram, {
                isDemo,
                liveCheck: options?.liveCheck ?? false
            });
        ...
        if (lead.facebook) {
            fbValidation = await SocialValidationService_1.SocialValidationService.validateFacebook(lead.facebook, {
                isDemo,
                liveCheck: options?.liveCheck ?? false
            });
        ...
        if (lead.googleBusinessUrl) {
            gmapsValidation = SocialValidationService_1.SocialValidationService.validateGoogleMaps(lead.googleBusinessUrl, { isDemo });
        ...
        if (lead.email) {
            ...
            if (emailRegex.test(cleanEmail)) {
                ...
                const domain = WebsiteDetectorService_1.WebsiteDetectorService.extractDomainFromEmail(cleanEmail);
...
exports.LeadValidationService = LeadValidationService;
