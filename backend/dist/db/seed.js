...
async function seed() {
    ...
    for (const n of PREDEFINED_NICHES) {
        await prisma_1.default.niche.upsert({
            where: { name: n.name },
    ...
    for (const lead of SEED_LEADS) {
        await prisma_1.default.lead.upsert({
    ...
    const firstNiche = await prisma_1.default.niche.findFirst({ where: { name: 'Barber' } });
    await prisma_1.default.search.create({
        data: {
