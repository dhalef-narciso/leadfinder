export type OutreachTone = 'Friendly' | 'Professional' | 'Direct' | 'Casual' | 'Short';
export type OutreachLanguage = 'en' | 'pt';
...
export class OutreachMessageGenerator {
  static generate(options: OutreachGenerationOptions): string {
    const {
      businessName,
      niche,
      location,
  ...
  private static generateEnglish(ctx: {
    businessName: string;
    nicheClean: string;
    location: string;
    contactName?: string | null;
...
Checked out your site on mobile and saw a few spots where you could easily get more inquiries. ${demoSentence}`;
  ...
  private static generatePortuguese(ctx: {
    businessName: string;
    nicheClean: string;
    location: string;
    contactName?: string | null;
