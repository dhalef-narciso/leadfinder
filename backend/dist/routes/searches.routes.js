...
const QueryGeneratorService_1 = require("../services/QueryGeneratorService");
...
router.post('/preview-queries', async (req, res) => {
    try {
        ...
        const nicheRecord = await prisma_1.default.niche.findFirst({ where: { name: niche } });
        ...
        const mapsQueries = QueryGeneratorService_1.QueryGeneratorService.generateMapsQueries(niche, customSynonyms, 4);
        // 2. Google Search Enrichment Queries
        const enrichmentQueries = QueryGeneratorService_1.QueryGeneratorService.generate({
            niche,
            locations,
...
router.post('/run', async (req, res) => {
    try {
        const { niche, locations, searchDepth = 1, numLeads = 25, includeGoogleSearch = true, includeInstagram = true, includeFacebook = true, includeWebsite = true, searchName } = req.body;
        if (!niche || !locations || !Array.isArray(locations) || locations.length === 0) {
            return res.status(400).json({ error: 'Niche and at least one location are required.' });
        }
        ...
        const mapsProvider = ProviderFactory_1.ProviderFactory.getMapsProvider();
        const allDiscoveredBusinesses = [];
        for (const loc of locations) {
            for (const queryVariation of mapsQueries) {
                if (allDiscoveredBusinesses.length >= targetLeadsCount * 1.5)
                    break;
        ...
        const extractedLeads = await LeadExtractorService_1.LeadExtractorService.extractFromMapsBusinesses(allDiscoveredBusinesses, niche, primaryLocation, {
            enrichment: {
                includeGoogleSearch,
                includeInstagram,
                includeFacebook,
        ...
        for (const item of finalLeads) {
            const saved = await prisma_1.default.lead.upsert({
        ...
        const savedSearch = await prisma_1.default.search.create({
            data: {
...
router.get('/', async (_req, res) => {
    try {
        const searches = await prisma_1.default.search.findMany({
...
router.post('/', async (req, res) => {
    try {
        ...
        const nicheRecord = await prisma_1.default.niche.findFirst({ where: { name: nicheName } });
        const search = await prisma_1.default.search.create({
            data: {
                name,
                nicheId: nicheRecord?.id,
                nicheName,
...
router.delete('/', async (_req, res) => {
    try {
        await prisma_1.default.searchQuery.deleteMany();
        const result = await prisma_1.default.search.deleteMany();
...
router.delete('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        await prisma_1.default.search.delete({ where: { id } });
