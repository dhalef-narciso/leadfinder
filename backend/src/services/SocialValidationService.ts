...
export interface SocialValidationResult {
  ...
  url: string | null;
  status: 'VALID' | 'INVALID' | 'UNREACHABLE' | 'UNVERIFIED' | 'NOT_CHECKED';
  handle: string | null;
  reason: string;
}

export class SocialValidationService {
  private static IG_RESERVED = new Set([
    'about', 'explore', 'accounts', 'developer', 'directory', 'legal', 'privacy', 'p', 'reel', 'stories', 'tv'
  ]);

  private static FB_RESERVED = new Set([
   ...
   */
  static async validateInstagram(
    url: string | null | undefined,
    options?: { isDemo?: boolean; liveCheck?: boolean }
    ...
    if (!UrlValidationService.isValidUrlFormat(cleanUrl)) {
      return {
        platform: 'instagram',
        url: cleanUrl,
        status: 'INVALID',
    ...
    try {
      ...
      const urlValidation = await UrlValidationService.validateUrl(cleanUrl, 'instagram', {
  ...
  static async validateFacebook(
    ...
    try {
      ...
      const urlValidation = await UrlValidationService.validateUrl(cleanUrl, 'facebook', {
        isDemo: options?.isDemo,
        liveCheck: true,
        timeoutMs: 3000
      });
