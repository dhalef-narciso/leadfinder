...
exports.LeadExtractorService = void 0;
const WebsiteDetectorService_1 = require("./WebsiteDetectorService");
const LeadScoringService_1 = require("./LeadScoringService");
const DeduplicationService_1 = require("./DeduplicationService");
const LeadValidationService_1 = require("./LeadValidationService");
const BusinessEnrichmentService_1 = require("./BusinessEnrichmentService");
class LeadExtractorService {
    /**
     * Cleans a search result title into a business name.
     * STRICT: Never fabricates a name if not present.
     */
    static cleanBusinessName(title) {
     ...
     */
    static async extractFromMapsBusinesses(businesses, niche, location, options) {
        // 1. Deduplicate raw Maps results using place ID, CID, phone, domain, normalized name + address
        const deduplicated = DeduplicationService_1.DeduplicationService.deduplicateMapsBusinesses(businesses, location);
        const leads = [];
        const seenKeys = new Set();
        for (const rawBiz of deduplicated) {
            // 2. Information Enrichment (Google Search, Social, Website classification)
            const enriched = await BusinessEnrichmentService_1.BusinessEnrichmentService.enrichBusiness(rawBiz, location, options?.enrichment || {});
            // 3. Generate deterministic identity key
            const identityKey = DeduplicationService_1.DeduplicationService.generateIdentityKey({
            ...
            // 4. Validate Lead
            const validation = await LeadValidationService_1.LeadValidationService.validateLead({
                businessName: enriched.businessName,
                niche,
                location,
            ...
            const scoreResult = LeadScoringService_1.LeadScoringService.calculate(scoreFactors, finalConfidence);
            leads.push({
     ...
     */
    static async extractFromResults(results, niche, location, options) {
        const leads = [];
        const seenKeys = new Set();
        for (const result of results) {
            ...
            const webDetection = WebsiteDetectorService_1.WebsiteDetectorService.detect(candidateWebUrl, result.snippet, email, { isDemo });
            const validation = await LeadValidationService_1.LeadValidationService.validateLead({
                businessName,
                niche,
                location,
            ...
            const scoreResult = LeadScoringService_1.LeadScoringService.calculate(scoreFactors, validation.validationConfidence);
            const identityKey = DeduplicationService_1.DeduplicationService.generateIdentityKey({
                businessName,
                location,
                websiteUrl: webDetection.url,
...
exports.LeadExtractorService = LeadExtractorService;
