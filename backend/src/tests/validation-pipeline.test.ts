...
import { WebsiteDetectorService } from '../services/WebsiteDetectorService';
import { SocialValidationService } from '../services/SocialValidationService';
...
async function runTests() {
  ...
  console.log('Test 1: Real business with website');
  {
    ...
    const leads = await LeadExtractorService.extractFromResults([rawResult], 'Plumber', 'Limerick');
    const lead = leads[0];
  ...
  console.log('\nTest 2: Real business without website');
  {
    ...
    const leads = await LeadExtractorService.extractFromResults([rawResult], 'Barber', 'Limerick');
    const lead = leads[0];
  ...
  console.log('\nTest 3: Business with Instagram only');
  {
    ...
    assert(
      lead.validationStatus === 'VERIFIED' || lead.validationStatus === 'POTENTIAL',
      `Valid Instagram handle qualifies lead as verified or potential (got: ${lead.validationStatus})`
    );
  }
  ...
  console.log('\nTest 4: Invalid Instagram URL');
  {
    const igValidation1 = await SocialValidationService.validateInstagram('https://www.instagram.com/about/');
    assert(igValidation1.status === 'INVALID', 'Reserved path /about/ is marked INVALID');

    const igValidation2 = await SocialValidationService.validateInstagram('https://www.instagram.com/invalid***handle$$/');
    assert(igValidation2.status === 'INVALID', 'Handle with invalid characters is marked INVALID');

    const igValidation3 = await SocialValidationService.validateInstagram('not-even-a-url');
  ...
  console.log('\nTest 5: Invalid Facebook URL');
  {
    const fbValidation1 = await SocialValidationService.validateFacebook('https://www.facebook.com/login/');
    assert(fbValidation1.status === 'INVALID', 'Reserved Facebook path /login/ is marked INVALID');

    const fbValidation2 = await SocialValidationService.validateFacebook('https://www.facebook.com/');
  ...
  console.log('\nTest 6: Fake/nonexistent business');
  {
    ...
    const leads = await LeadExtractorService.extractFromResults([rawResult], 'Services', 'Limerick');
    const lead = leads[0];
  ...
  console.log('\nTest 7: Business with email domain');
  {
    ...
    const domain = WebsiteDetectorService.extractDomainFromEmail(email);
    ...
    const gmailDomain = WebsiteDetectorService.extractDomainFromEmail('barber@gmail.com');
    ...
    const detection = WebsiteDetectorService.detect(null, 'Commercial cleaning contracts.', email);
    assert(
      detection.status === 'Website Unclear',
      `Custom email domain classified as "Website Unclear", NOT "Website Not Found" (got: ${detection.status})`
    );
  ...
  console.log('\nTest 8: Duplicate business');
  {
    ...
    const leads = await LeadExtractorService.extractFromResults(rawResults, 'Barber', 'Limerick');
  ...
  console.log('\nTest 9: Search result with missing phone');
  {
    ...
    const leads = await LeadExtractorService.extractFromResults([rawResult], 'Tattoo', 'Limerick');
    const lead = leads[0];
  ...
  console.log('\nBonus Check: Opportunity Score Safety Rules');
  {
    ...
    const lowConfResult = LeadScoringService.calculate(
      {
    ...
    assert(
      lowConfResult.score <= 30,
      `Validation < 50 caps Opportunity Score to max 30 (got: ${lowConfResult.score})`
    );
    assert(lowConfResult.tier === 'Low', `Score tier is Low when capped (got: ${lowConfResult.tier})`);

    // Moderate validation confidence (50-69)
    const modConfResult = LeadScoringService.calculate(
      {
        hasNoWebsite: true,
        hasInstagram: true,
        hasFacebook: true,
    ...
    assert(
      modConfResult.score <= 60,
      `Validation 50-69 caps Opportunity Score to max 60 (got: ${modConfResult.score})`
    );
