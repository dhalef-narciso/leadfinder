...
import { SocialValidationService } from './SocialValidationService';
...
export interface FieldEvidenceItem {
  value: string | null;
  source: 'google_maps' | 'google_search' | 'website_domain' | 'none';
  status: 'verified' | 'unverified' | 'found' | 'no_website_detected' | 'not_found';
  verifiedAt?: string;
  notes?: string;
}
...
export interface EnrichedBusinessData {
  ...
  websiteStatus: 'Website Found' | 'Website Not Found' | 'Website Unclear';
  ...
  fieldEvidence: Record<string, FieldEvidenceItem>;
...
export class BusinessEnrichmentService {
   ...
   */
  static async enrichBusiness(
    mapsBusiness: RawMapsBusiness,
    location: string,
    options: EnrichmentOptions = {}
  ): Promise<EnrichedBusinessData> {
    const evidence: Record<string, FieldEvidenceItem> = {};

    let websiteUrl: string | null = null;
    let websiteStatus: 'Website Found' | 'Website Not Found' | 'Website Unclear' = 'Website Not Found';
    let instagram: string | null = null;
    let facebook: string | null = null;
    let phone: string | null = mapsBusiness.phone || null;
    ...
    if (mapsBusiness.website) {
      ...
      if (lowerWeb.includes('instagram.com')) {
        const igCheck = await SocialValidationService.validateInstagram(rawWeb, { isDemo: mapsBusiness.isDemo });
        ...
        const fbCheck = await SocialValidationService.validateFacebook(rawWeb, { isDemo: mapsBusiness.isDemo });
      ...
      } else if (UrlValidationService.isValidUrlFormat(rawWeb)) {
        websiteUrl = rawWeb;
    ...
    if (shouldEnrich && (!websiteUrl || !instagram || !facebook || !email)) {
      try {
        const searchProvider = ProviderFactory.getProvider();
        ...
        for (const res of searchResults) {
          ...
          if (!instagram && options.includeInstagram && res.link && res.link.includes('instagram.com')) {
            const igCheck = await SocialValidationService.validateInstagram(res.link);
            if (igCheck.status !== 'INVALID' && igCheck.url) {
              instagram = igCheck.url;
              evidence.instagram = {
                value: instagram,
          ...
          if (!facebook && options.includeFacebook && res.link && res.link.includes('facebook.com')) {
            const fbCheck = await SocialValidationService.validateFacebook(res.link);
            if (fbCheck.status !== 'INVALID' && fbCheck.url) {
              facebook = fbCheck.url;
              evidence.facebook = {
                value: facebook,
